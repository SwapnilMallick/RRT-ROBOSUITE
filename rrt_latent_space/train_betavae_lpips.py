"""
Beta-VAE for 84x84x3 robomimic images, trained with an MS-SSIM + LPIPS
reconstruction loss and KL annealing.

Loss = alpha * (1 - MS_SSIM(x, x_hat)) + (1 - alpha) * LPIPS(x, x_hat) + beta(t) * KL

Rationale:
  * MS-SSIM scores multi-scale structure (edges of arm/cube) instead of the
    per-pixel mean MSE collapses toward -> crisper reconstructions.
  * LPIPS (Learned Perceptual Image Patch Similarity) replaces L1: it measures
    distance in deep feature space (AlexNet by default), making it sensitive to
    semantic/perceptual differences rather than per-pixel misalignment.  This
    typically yields sharper, less blurry reconstructions than L1/MSE.
...
LPIPS note: the network expects images in [-1, 1]; internally we remap from [0, 1].
...
def recon_loss(x, x_hat, alpha, lpips_fn):
    """
    x, x_hat in [0, 1].  LPIPS network expects [-1, 1], so we remap internally.
    Returns (total_recon, dssim_scalar, lpips_scalar).
    """
    dssim = 1.0 - ms_ssim(x_hat, x)
    # remap [0,1] -> [-1,1] for LPIPS
    x_lp    = x     * 2.0 - 1.0
    xhat_lp = x_hat * 2.0 - 1.0
    lpips_val = lpips_fn(xhat_lp, x_lp).mean()
    return alpha * dssim + (1.0 - alpha) * lpips_val, dssim.item(), lpips_val.item()
...
def save_recon_grid(x, x_hat, beta, epoch, img_dir):
    ...
    fig.suptitle(f"Beta-VAE (LPIPS)  |  beta={beta:.4f}  |  epoch={epoch:03d}", fontsize=10)
    for i in range(n):
...
def train(cfg):
    ...
    beta_tag = f"beta_{cfg['beta']}_lpips"
    ...
    header = (f"device {dev} | frames {len(ds)} | latent {cfg['latent']} | "
              ...
              f"lpips_net {cfg['lpips_net']}")
    log(header)

    best_loss = float("inf")
    best_ckpt = os.path.join(cfg["ckpt_dir"], f"betavae_{cfg['beta']}_lpips.pt")

    for ep in range(cfg["epochs"]):
        model.train()
        beta = beta_at(ep, cfg["beta"], cfg["anneal"])
        agg = {"loss": 0, "dssim": 0, "lpips": 0, "kl": 0, "nb": 0}
        last_x = last_xhat = None
        for x in dl:
            x = x.to(dev)
            x_hat, mu, logvar = model(x)
            rec, dssim, lpips_val = recon_loss(x, x_hat, cfg["alpha"], lpips_fn)
            kl = kl_per_dim(mu, logvar)
            loss = rec + beta * kl
            opt.zero_grad(); loss.backward(); opt.step()
            for k, v in (("loss", loss.item()), ("dssim", dssim), ("lpips", lpips_val),
                         ("kl", kl.item()), ("nb", 1)):
                agg[k] += v
        ...
        log(f"ep {ep:03d} | beta {beta:.3f} | loss {ep_loss:.4f} | "
            f"1-msssim {agg['dssim']/nb:.4f} | LPIPS {agg['lpips']/nb:.4f} | "
            f"KL/dim {agg['kl']/nb:.4f}")

        # rolling last checkpoint
        torch.save({"model": model.state_dict(), "cfg": cfg, "epoch": ep},
                   os.path.join(cfg["ckpt_dir"], f"betavae_{cfg['beta']}_lpips_last.pt"))
...
if __name__ == "__main__":
    ...
    p.add_argument("--alpha", type=float, default=0.85)     # MS-SSIM vs LPIPS weight
    ...
    if a.smoke_test:
        ...
        rec, dssim, lpips_val = recon_loss(x, x_hat, a.alpha, lpips_fn)
        kl = kl_per_dim(mu, lv)
        (rec + kl).backward()
        ...
        print(f"[smoke] x_hat {tuple(x_hat.shape)} | recon {rec.item():.4f} "
              f"(1-msssim {dssim:.4f}, LPIPS {lpips_val:.4f}) | KL/dim {kl.item():.4f} "
